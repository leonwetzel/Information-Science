...
Modified by Leon Wetzel on 26th of March, 2017
s3284174
...
def extract_features(words, lemmas, tags, n0, n, stack, parse):
    ...
    #### ASSIGNMENT 3 - part B: adding features to a transition-based parser ####
    ...
    # For first word on buffer, get valency and words/tags of left/right daughters
    ...
    # For word on top of stack, get valency and words/tags of left/right daughters
    ...
    # Valences
    # Left and right valences from buffer
    if b_l_valence: features.append(('b_l_valence=%s' % (b_l_valence), 1))
    if b_r_valence: features.append(('b_r_valence=%s' % (b_r_valence), 1))
    # Left and right valences from stack
    if s_l_valence: features.append(('s_l_valence=%s' % (s_l_valence), 1))
    if s_r_valence: features.append(('s_r_valence=%s' % (s_r_valence), 1))

    # Combined valences from buffer and stack, plus valence and word/tag combinations
    if b_l_valence and s_l_valence: features.append(('b_l_valence=%s s_l_valence=%s' % (b_l_valence, s_l_valence), 1))
    if b_r_valence and s_r_valence: features.append(('b_r_valence=%s s_r_valence=%s' % (b_r_valence, s_r_valence), 1))
    if b_l_valence and b_l_w0: features.append(('b_l_valence=%s b_l_w0=%s' % (b_l_valence, b_l_w0), 1))
    if b_r_valence and b_l_w0: features.append(('b_r_valence=%s b_l_w0=%s' % (b_r_valence, b_l_w0), 1))
    if b_r_valence and s_r_w0: features.append(('b_r_valence=%s s_r_w0=%s' % (b_r_valence, s_r_w0), 1))
    if b_r_valence and s_w1: features.append(('b_r_valence=%s s_w1=%s' % (b_r_valence, s_w1), 1))

    # FIX: does not improve UAS
    # if b_l_valence and b_l_p0: features.append(('b_l_valence=%s b_l_p0=%s' % (b_l_valence, b_l_p0), 1))
    # if b_l_valence and b_l_w1: features.append(('b_l_valence=%s b_l_w1=%s' % (b_l_valence, b_l_w1), 1))
    # if s_l_valence and b_l_w0: features.append(('s_l_valence=%s b_l_w0=%s' % (s_l_valence, b_l_w0), 1))
    # if s_l_valence and s_l_w0: features.append(('s_l_valence=%s s_l_w0=%s' % (s_l_valence, s_l_w0), 1))
    # if s_l_valence and s_l_w1: features.append(('s_l_valence=%s s_l_w1=%s' % (s_l_valence, s_l_w1), 1))
    # if b_l_valence and s_w0: features.append(('b_l_valence=%s s_w0=%s' % (b_l_valence, s_w0), 1))
    # if b_r_valence and s_w0: features.append(('b_r_valence=%s s_w0=%s' % (b_r_valence, s_w0), 1))
    # if b_l_valence and s_w1: features.append(('b_l_valence=%s s_w1=%s' % (b_l_valence, s_w1), 1))

    if b_r_valence and b_w0: features.append(('b_r_valence=%s b_w0=%s' % (b_r_valence, b_w0), 1))
    if b_r_valence and b_p0: features.append(('b_r_valence=%s b_p0=%s' % (b_r_valence, b_p0), 1))

    # Context features
    # Left daughters of words
    if b_l_w0: features.append(('b_l_w0=%s' % (b_l_w0), 1))
    if b_l_w1: features.append(('b_l_w1=%s' % (b_l_w1), 1))
    # Left daughters of POS tags
    if b_l_p0: features.append(('b_l_p0=%s' % (b_l_p0), 1))
    if b_l_p1: features.append(('b_l_p1=%s' % (b_l_p1), 1))
    # FIX: does not improve UAS
    # Right daughters of words
    # if b_r_w0: features.append(('b_r_w0=%s' % (b_r_w0), 1))
    # if b_r_w1: features.append(('b_r_w1=%s' % (b_r_w1), 1))
    # if b_r_p0: features.append(('b_r_p0=%s' % (b_r_p0), 1))
    # if b_r_p1: features.append(('b_r_p1=%s' % (b_r_p1), 1))

    # Combined word and tags branches
    if b_l_w0 and b_l_p0: features.append(('b_l_w0=%s b_l_p0=%s' % (b_l_w0, b_l_p0), 1))
    if b_l_w1 and b_l_p1: features.append(('b_l_w1=%s b_l_p1=%s' % (b_l_w1, b_l_p1), 1))
    if b_l_w0 and b_l_p1: features.append(('b_l_w0=%s b_l_p1=%s' % (b_l_w0, b_l_p1), 1))
    # FIX: does not improve UAS
    # if b_l_w1 and b_l_p0: features.append(('b_l_w1=%s b_l_p0=%s' % (b_l_w1, b_l_p0), 1))

    # Top word/tag on stack
    # Left daughters
    ...
    # Right daughters
    if s_r_w0: features.append(('s_r_w0=%s' % (s_r_w0), 1))
    if s_r_w1: features.append(('s_r_w1=%s' % (s_r_w1), 1))
    if s_r_p0: features.append(('s_r_p0=%s' % (s_r_p0), 1))
    if s_r_p1: features.append(('s_r_p1=%s' % (s_r_p1), 1))

    # Combined right branches from tags and words
    if s_r_p0 and s_r_w0: features.append(('s_r_p0=%s s_r_w0=%s' % (s_r_p0, s_r_w0), 1))
    if s_r_p1 and s_r_w1: features.append(('s_r_p1=%s s_r_w1=%s' % (s_r_p1, s_r_w1), 1))
    # Combined left branches from tags and words
    if s_l_p0 and s_l_w0: features.append(('s_l_p0=%s s_l_w0=%s' % (s_l_p0, s_l_w0), 1))
    if s_l_p1 and s_l_w1: features.append(('s_l_p1=%s s_l_w1=%s' % (s_l_p1, s_l_w1), 1))

    # Unigram features
    # Word features (including position) - on stack
    ...
    if s_w0 and s_r_p0: features.append(('s_w0=%s s_r_p0=%s' % (s_w0, s_r_p0), 1))
    if b_w0 and s_r_p0: features.append(('b_w0=%s s_r_p0=%s' % (b_w0, s_r_p0), 1))
    # FIX: does not improve UAS
    # if b_p0 and s_r_p0: features.append(('b_p0=%s s_r_p0=%s' % (b_p0, s_r_p0), 1))
    # if b_w0 and s_l_p0: features.append(('b_w0=%s s_l_p0=%s' % (b_w0, s_l_p0), 1))
    # if s_p0 and s_r_p0: features.append(('s_p0=%s s_r_p0=%s' % (s_p0, s_r_p0), 1))
    # if s_w0 and s_l_p0: features.append(('s_w0=%s s_l_p0=%s' % (s_w0, s_l_p0), 1))
    # if s_w1 and s_r_p1: features.append(('s_w1=%s s_r_p1=%s' % (s_w1, s_r_p1), 1))
    ...
    if s_w0 and b_p0: features.append(('s_w0=%s b_p0=%s' % (s_w0, b_p0), 1))
    if s_w1 and b_p1: features.append(('s_w1=%s b_p1=%s' % (s_w1, b_p1), 1))
    if s_w2 and b_p2: features.append(('s_w2=%s b_p2=%s' % (s_w2, b_p2), 1))
    ...
    if b_p0 and s_p0: features.append(('b_p0=%s s_p0=%s' % (b_p0, s_p0), 1))
    if b_p1 and s_p1: features.append(('b_p1=%s s_p1=%s' % (b_p1, s_p1), 1))
    if b_p2 and s_p2: features.append(('b_p2=%s s_p2=%s' % (b_p2, s_p2), 1))

    if s_w0 and s_p0 and b_w0 and b_p0: features.append(('s_w0=%s s_p0=%s b_w0=%s b_p0=%s' % (s_w0, s_p0, b_w0, b_p0), 1))
    if s_w1 and s_p1 and b_w1 and b_p1: features.append(('s_w1=%s s_p1=%s b_w1=%s b_p1=%s' % (s_w1, s_p1, b_w1, b_p1), 1))
    if s_w2 and s_p2 and b_w2 and b_p2: features.append(('s_w2=%s s_p2=%s b_w2=%s b_p2=%s' % (s_w2, s_p2, b_w2, b_p2), 1))
...
def test_extraction():
    """
    A function that shows an example configuration / for testing/exemplar purposes only
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output-gold", help="output gold head to file", required=False, action="store_true", default=False)
    parser.add_argument("--show-example-configuration", help="show example configuration", required=False, action="store_true", default=False)
    ...
    if args.show_example_configuration:
        test_extraction()
